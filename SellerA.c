...
int main(){
	...
	//while(1){
        ...
        if ((fp = fopen("sellerA.txt", "r")) == NULL)
  { /* Open source file. */
...
i++;
 ...
 //sleep(5);
	...
	printf("<SellerA> has TCP port %u and IP address %s for Phase 1 part 1\n\n", myPort, myIP);
	printf("<SellerA> is now connected to the <agent1>\n\n");

        int nu=send(clientSocket, buffer[1], 1024, 0);
		if(nu==-1)
		{
			fprintf(stderr, "Failure Sending Message\n");
            close(clientSocket);
     ...
     printf("<SellerA> has sent <Seller: %s, %s, %s > to the agent\n\nEnd of Phase 1 part 1 for <SellerA>\n\n",buffer[0],buffer[1],buffer[2]);
...
printf("<SellerA> has TCP port %d and IP address %s for Phase 1 part 4\n\n",ntohs(server.sin_port),inet_ntoa(server.sin_addr));
...
printf("End of Phase 1 part 4for <SellerA>");
