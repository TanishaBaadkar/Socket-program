...
int main(){
	...
	//while(1){
        ...
        if ((fp = fopen("sellerC.txt", "r")) == NULL)
  { /* Open source file. */
...
i++;
 ...
 //sleep(5);
	...
	printf("<SellerC> has TCP port %u and IP address %s for Phase 1 part 1\n\n", myPort, myIP);
	printf("<SellerC> is now connected to the <agent2>\n\n");
	 ...
	 printf("<SellerC> has sent <Seller: %s, %s, %s > to the agent\n\nEnd of Phase 1 part 1 for <SellerC>\n\n",buffer[0],buffer[1],buffer[2]);	
...
printf("<SellerC> has TCP port %d and IP address %s for Phase 1 part 4\n\n",ntohs(server.sin_port),inet_ntoa(server.sin_addr));
...
printf("End of Phase 1 part 4for <SellerC>");
