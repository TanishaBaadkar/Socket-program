...
printf("The <Buyer4>has UDPport %d and IP address %s for Phase 1 part 2\n\n",ntohs(servaddr.sin_port),inet_ntoa(servaddr.sin_addr));     int i;
...
//if(m==0)
 ...
 printf("End of Phase 1 part 2for <Buyer4>\n\n");
...
char buffer2[2][1024];
    char buf1[1024];
     ...
     if ((fp2 = fopen("buyer4.txt", "r")) == NULL)
  { /* Open source file. */
    perror("fopen source-file");
    return 1;
  }
...
for (q=0; q<strlen(result1); q++)
{
    ...
    printf("The<Buyer4>has TCPport %d and IP address %s for Phase 1 part 3\n\n", myPort, myIP);

    printf("<Buyer4> has sent\n< %s > to <Agent1>\n\n",result1);
...
for (q=0; q<strlen(result2); q++)
{
 ...
 printf("<Buyer4> has sent\n< %s > to <Agent2>\n\n",result2);
 printf("End of Phase 1 part 3 for <Buyer4>\n\n");
//PART4
...
if(count==0)
    o=2;
if(count==1||count==0)
printf("<Buyer4> has TCPport %d and IP address %s for Phase 1 part 4\n\n",ntohs(sAddr.sin_port),inet_ntoa(sAddr.sin_addr));
    for(i1=0;i1<o;i1++)
    {
...
if(count==1||count==0)
printf("End of Phase 1 part 4for <Buyer4>");
exit(0); 
}
