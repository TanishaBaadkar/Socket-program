...
#define PORT6 4231 //part4-sellerC
#define PORT7 4331 //part4-sellerD
...
int main(){
	...
	printf("The <Agent2>has TCPport %d and IP address %s for Phase 1 part 1\n\n",ntohs(serverAddr.sin_port),inet_ntoa(serverAddr.sin_addr)); 
...
printf("End of Phase 1 part 1 for <Agent2>\n\n");
...
if ((he = gethostbyname("nunki.usc.edu"))==NULL) {
	...
	printf("The<Agent2> has UDPport %d and IP address %s for Phase 1 part 2\n\n", myPort, myIP);
	 printf("<Agent2> has sent < %s > to <Buyer1>\n\n",str2);
    ...
    sendto(sockfd3, (char *)buffer2, 1024,0, (const struct sockaddr *) &ser1, sizeof(ser1));
     printf("<Agent2> has sent < %s > to <Buyer2>\n\n",str2);
    ...
    sendto(sockfd4, (char *)buffer2, 1024,0, (const struct sockaddr *) &ser2, sizeof(ser2));
     printf("<Agent2> has sent < %s > to <Buyer3>\n\n",str2);
    ...
    sendto(sockfd5, (char *)buffer2, 1024,0, (const struct sockaddr *) &ser3, sizeof(ser3));
     printf("<Agent2> has sent < %s > to <Buyer4>\n\n",str2);
...
///fifth
    ...
    sendto(sockfd6, (char *)buffer2, 1024,0, (const struct sockaddr *) &ser4, sizeof(ser4));
     printf("<Agent2> has sent < %s > to <Buyer5>\n\n",str2);

printf("End of Phase 1 part 2 for <Agent2>\n\n");
    //PART3
  ...
  char buffer1[2024];
	...
	printf("The<Agent2> has TCPport %d and IP address %s for Phase 1 part 3\n\n",ntohs(serverAddr.sin_port),inet_ntoa(serverAddr.sin_addr));
		...
		if((childpid1 = fork()) == 0)
		{
			//close(sockfd);
				...
				recv(newSocket, buffer1, 2024, 0);
					...
					{
						printf("<Agent2>receives the offer from <Buyer 1>\n\n");
					fp1=fopen("A2B1.txt", "w+");
				    }
				    if(strncmp(&buffer1[0],"2",1)==0)
				    {
				    	printf("<Agent2>receives the offer from <Buyer 2>\n\n");
					fp1=fopen("A2B2.txt", "w+");
				}
				    if(strncmp(&buffer1[0],"3",1)==0)
				    {
				    	printf("<Agent2>receives the offer from <Buyer 3>\n\n");
					fp1=fopen("A2B3.txt", "w+");
				}
				    if(strncmp(&buffer1[0],"4",1)==0)
				    {
				    	printf("<Agent2>receives the offer from <Buyer 4>\n\n");
					fp1=fopen("A2B4.txt", "w+");
				}
				    if(strncmp(&buffer1[0],"5",1)==0)
				    {
					fp1=fopen("A2B5.txt", "w+");
                    printf("<Agent2>receives the offer from <Buyer 5>\n\n");
                }
...
printf("End of Phase 1 part 3 for <Agent2>\n\n");
...
for(r=0;r<5;r++)
{
if(strncmp(buyer[r].p1,"C",1)==0)
{
	buyersC[t].name=buyer[r].name;
	buyersC[t].budget=buyer[r].budget;
	t++;
}
}
...
for(r=0;r<5;r++)
{
if(strncmp(buyer[r].p1,"D",1)==0)
{
	buyersD[t].name=buyer[r].name;
	buyersD[t].budget=buyer[r].budget;
	t++;
}
}
...
char resultC[100];
if(buyersC[0].budget==0)
	strcpy(resultC,"NAK");

char resultD[100];
if(buyersD[0].budget==0)
	strcpy(resultD,"NAK");
...
for(t=0;t<10;t++)
{
	if(buyersC[t].budget>costC)
	{
		costC=buyersC[t].budget;
		u=nameC;
		nameC=buyersC[t].name;
	}
}
...
for(t=0;t<10;t++)
{
	if(buyersD[t].budget>costD)
	{
		costD=buyersD[t].budget;
		r=nameD;
		nameD=buyersD[t].name;
	}
}

if(nameC==nameD)
 {
 	if(strncmp(buyer[nameC-1].p1,"C",1)==0)
 	{
 		nameD=r;
 	}

 	if(strncmp(buyer[nameC-1].p1,"D",1)==0)
 	{
 		nameC=u;
 	}
 }
//printf("nameC%d",nameC);
//printf("nameD%d",nameD);

if(nameC==0)
strcpy(resultC,"NAK");

else 
	sprintf(resultC, "%d", nameC);

if(nameD==0)
strcpy(resultD,"NAK");
else
	sprintf(resultD, "%d", nameD);
...
//struct sockaddr_in s;
    ...
    if ((send(sockfd,resultC, strlen(resultC),0))== -1)
         {
	...
	printf("The<Agent2> has TCP port %d and IP address %s for Phase 1 part 4\n\n", myPort, myIP);

	printf("The <Agent2> has send the result to <SellerA>\n\n");
    ...
    if ((send(sockfd2,resultD, strlen(resultD),0))== -1)
         {
            fprintf(stderr, "Failure Sending Message\n");
            close(sockfd2);
            exit(1);
        ...
        printf("The <Agent2> has send the result to <SellerB>\n\n");
...
printf("The <Agent2> has send the result to <Buyer1>\n\n");
close(sockfd2);
 }
sleep(5);
...
printf("The <Agent2> has send the result to <Buyer2>\n\n");
close(sockfd2);
 }
sleep(5);
...
printf("The <Agent2> has send the result to <Buyer3>\n\n");
close(sockfd2);
 }
sleep(5);
...
printf("The <Agent2> has send the result to <Buyer4>\n\n");
close(sockfd2);
 }
sleep(5);
...
printf("The <Agent2> has send the result to <Buyer5>\n\n");
close(sockfd2);
 }
printf("End of Phase 1 part 4 for <Agent2>");
	exit(0);
}