...
//sleep(5);
	...
	//while(1){
        ...
        if ((fp = fopen("sellerB.txt", "r")) == NULL)
  { /* Open source file. */
...
if(send(clientSocket, buffer[0], 4, 0)==-1)
	...
	printf("<SellerB> has TCP port %u and IP address %s for Phase 1 part 1\n\n", myPort, myIP);
	printf("<SellerB> is now connected to the <agent1>\n\n");
		...
		if(send(clientSocket, buffer[2], 1024, 0)==-1)
		{
		 ...
		 printf("<SellerB> has sent <Seller: %s, %s, %s > to the agent\n\nEnd of Phase 1 part 1 for <SellerB>\n\n",buffer[0],buffer[1],buffer[2]);
...
printf("<SellerB> has TCP port %d and IP address %s for Phase 1 part 4\n\n",ntohs(server.sin_port),inet_ntoa(server.sin_addr));
...
printf("End of Phase 1 part 4 for <SellerB>");
